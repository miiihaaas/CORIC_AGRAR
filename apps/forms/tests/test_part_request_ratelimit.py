"""Story 4.5 — AC9: security must-haves (ratelimit → HTTP 429) — TEA RED phase.

Pokriva AC9 / Task 4.5 (REUSE 4.2/4.4 SM-D9):
...
Pokrenuti:
    just test apps/forms/tests/test_part_request_ratelimit.py -v

Refs: 4-5 AC9 + Task 4.5 + SM-D9; interface-contract § 9.
"""
...
# AC-9: prvih 5 submit-a sa istog IP-a PROLAZI (status NIJE 429)
def test_first_five_submits_pass(
    htmx_post, part_request_submit_url, part_request_payload, recipient_env, mailoutbox
):
    for i in range(5):
        response = htmx_post(
            part_request_submit_url, part_request_payload, ip="198.51.100.5"
        )
        assert response.status_code != 429, (
            f"Submit #{i + 1} od 5 NE SME biti ograničen (rate=5/m; AC9), dobio {response.status_code}."
        )


# AC-9: 6. uzastopni submit sa istog IP-a u 1 min → HTTP 429 (NE 403)
def test_sixth_submit_returns_429(
    htmx_post, part_request_submit_url, part_request_payload, recipient_env, mailoutbox
):
    statuses = [
        htmx_post(part_request_submit_url, part_request_payload, ip="198.51.100.6").status_code
        for _ in range(6)
    ]
    ...
    assert 403 not in statuses, (
        ...
        "umesto block=False (AC9/SM-D9). Story ostaje RED dok Dev ne ispravi decorator."
    )


# AC-9: različiti IP-ovi se broje odvojeno (key='ip')
def test_ratelimit_is_per_ip(
    htmx_post, part_request_submit_url, part_request_payload, recipient_env, mailoutbox
):
    for _ in range(5):
        htmx_post(part_request_submit_url, part_request_payload, ip="198.51.100.10")
    response = htmx_post(part_request_submit_url, part_request_payload, ip="198.51.100.20")
    assert response.status_code != 429, (
        "Ratelimit je key='ip' — drugi IP NE SME biti ograničen brojačem prvog IP-a (AC9)."
    )
