...
def test_sections_render_in_correct_order(client):
    ...
    galerija → specs → brošura → testimonijali → slični → variants TAČNIM redosledom
    (verifikuje monotono rastuće position indices u HTML render-u).

    SM: redosled promenjen (galerija PRE specs-a; testimonijali PRE sličnih modela) da
    ...
    assert (
        ...
        "Očekivan (redizajn): hero < opis < galerija < specs < brošura < testimonijali < slični < variants."
    )
